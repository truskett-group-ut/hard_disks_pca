...
def Compress(eta_tgt, scale=0.999):
    ...
    if rank == 0:
        ...
        Vp = sum(pi*(snap.particles.diameter**3.0)/6.0)
...
d = 1.0
etas = arange(0.40, 0.700001, 0.005)
a = (((pi*d**3.0)/6.0)/etas[0])**(1.0/3.0)
n = 16    
    
system = hoomd.init.create_lattice(unitcell=hoomd.lattice.sc(a=a), n=n)
...
for eta in etas:
    ...
    d = hoomd.dump.gsd("./trajectories/trajectory_{:.4f}.gsd".format(eta), 
